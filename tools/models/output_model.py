...
class ExplorationLink(BaseModel):
    ...
    url: str | None = Field(
        None,
        description=(
            "Pre-populated URL with known context values (temporal range, spatial bounding box, "
            "collection concept ID) already substituted. Use directly to construct an 'Open in X' "
            "action. Links derived from a PotentialAction have parameters pre-filled; others point "
            "to the tool's home page. "
    ...
    topic: str | None = Field(
        None,
        description=(
            "Tool topic from UMM-T ToolKeywords (e.g. 'DATA ANALYSIS AND VISUALIZATION', "
            "'DATA DISCOVERY'). Use to group or label exploration links by capability."
        ),
    )
